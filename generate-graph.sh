cat log.txt |grep -i "val_acc:.*"|awk 'BEGIN{i=1}{print i, $14*100; i=i+1;}'|head -n 50 > rnn-train.txt
cat log.txt |grep -i "val_acc:.*"|awk 'BEGIN{i=-49}{print i, $14*100; i=i+1;}'|head -n 100|tail -n 50 > gru-train.txt
cat log.txt |grep -i "val_acc:.*"|awk 'BEGIN{i=-99}{print i, $14*100; i=i+1;}'|tail -n 50 > lstm-train.txt
